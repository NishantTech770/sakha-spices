...
const SkeletonRow = ({ cols }) => (
  <tr>{Array.from({ length: cols }).map((_, i) => <td key={i}><div className="ad-skeleton" /></td>)}</tr>
);
...
export default function Dashboard() {
  ...
  const stats = [
    { label: "Total Users",     value: loadingU ? "…" : users.length,                       icon: <UsersIcon />, color: "#e8821a" },
    { label: "Total Messages",  value: loadingM ? "…" : messages.length,                    icon: <MsgIcon />,   color: "#1b5e20" },
    { label: "Admin Users",     value: loadingU ? "…" : users.filter(u=>u.role==="admin").length, icon: <AdminIcon />, color: "#6d3200" },
    { label: "Messages (Month)",value: loadingM ? "…" : thisMonthMsgs.length,               icon: <CalIcon />,   color: "#c46a0e" },
  ];
    ...
    <div className="ad-content">
      ...
      <div className="ad-card">
        <div className="ad-card__header">
          ...
          <button className="ad-link-btn" onClick={() => navigate("/admin/messages")}>View All →</button>
        </div>
        <div className="ad-table-wrap">
          <table className="ad-table">
            <thead><tr><th>Name</th><th>Email</th><th>Category</th><th>Date</th></tr></thead>
            <tbody>
              {loadingM
                ...
                : messages.slice(0,5).map((m,i) => (
                  <tr key={i}>
                    <td>{m.name}</td>
                    <td>{m.email}</td>
                    <td><span className="ad-badge ad-badge--msg">{m.category}</span></td>
      ...
      <div className="ad-card">
        <div className="ad-card__header">
          ...
          <button className="ad-link-btn" onClick={() => navigate("/admin/users")}>View All →</button>
        </div>
        <div className="ad-table-wrap">
          <table className="ad-table">
              ...
              {loadingU
                ...
                : users.slice(0,5).map((u,i) => (
                  <tr key={i}>
                    ...
                    <td>{u.email}</td>
                    <td>{u.phone}</td>
                    <td><span className={`ad-badge ${u.role==="admin"?"ad-badge--admin":"ad-badge--user"}`}>{u.role}</span></td>
                  </tr>
                ))
              }
            </tbody>
